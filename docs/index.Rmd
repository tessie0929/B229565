...
pres_joined <- pitc2024_all %>% 
  ...
  mutate(
    year  = PaidDateMonth %/% 100, # extract calendar year from YYYYMM format
...
mh_by_simd_2024 <- mh_rate_gp_2024 %>% 
  ...
  summarise(
    mean_rate   = mean(rate_per_1000, na.rm = TRUE), # average prescribing rate per quintile
    median_rate = median(rate_per_1000, na.rm = TRUE), # median rate (less sensitive to outliers)
    sd_rate     = sd(rate_per_1000, na.rm = TRUE), # variability in rates within each quintile
    n_gp        = n(), # number of GP practices in each quintile
    .groups     = "drop"
  )
```

# Results
Mental health prescribing rates were analysed at GP practice level. For each practice, the total number of BNF 04.01, 04.02 and 04.03 items issued over 2024 was summed and divided by the registered list size to obtain a prescribing rate per 1000 patients. Descriptive statistics (mean, median, standard deviation and number of practices) were calculated within each SIMD quintile, and differences in distributions were visualised using boxplots. Monthly mean prescribing rates were then derived by aggregating practice-level rates within each SIMD quintile for every calendar month in 2024, allowing assessment of seasonal patterns and persistence of the deprivation gradient. Prescribing was also stratified by drug class (antidepressants, antipsychotics, anxiolytics/hypnotics) to examine whether similar socioeconomic patterns were present across different types of mental health medication. Finally, a Pearson correlation was computed between SIMD quintile (treated as an ordinal variable) and prescribing rate per 1000 patients to quantify the overall strength and direction of the association between socioeconomic deprivation and mental health prescribing.
...
ggplot(mh_rate_gp_2024,
  ...
  geom_boxplot(outlier.alpha = 0.3) + #Set the y-axis range to make the graph larger
